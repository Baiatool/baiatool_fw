...
{
    ARG_UNUSED(fixture);
    baiatool_storage_delete(BAIATOOL_AUTHENTICATION_NVS_ID);
...
ZTEST(storage, test_read_null_buffer_returns_einval)
{
    ssize_t ret = baiatool_storage_read(BAIATOOL_AUTHENTICATION_NVS_ID, NULL, 4U);

    zassert_equal(ret, -EINVAL, "expected -EINVAL, got %d", ret);
}

ZTEST(storage, test_read_nonexistent_returns_error)
{
    uint32_t buf = 0U;

    ssize_t ret = baiatool_storage_read(BAIATOOL_NET_SETTINGS_NVS_ID, &buf, sizeof(buf));

    zassert_true(ret < 0, "expected error for nonexistent ID, got %d", ret);
}

ZTEST(storage, test_create_returns_bytes_written)
{
    uint32_t data = 0xDEADBEEFU;

    ssize_t ret = baiatool_storage_create(BAIATOOL_AUTHENTICATION_NVS_ID, &data, sizeof(data));

    zassert_true(ret > 0, "create returned %d, expected > 0", ret);
}

ZTEST(storage, test_create_then_read_data_matches)
{
    uint32_t written = 0xCAFEBABEU;
    uint32_t read_back = 0U;

    baiatool_storage_create(BAIATOOL_AUTHENTICATION_NVS_ID, &written, sizeof(written));
    ssize_t ret = baiatool_storage_read(BAIATOOL_AUTHENTICATION_NVS_ID, &read_back, sizeof(read_back));

    zassert_true(ret > 0, "read failed: %d", ret);
    zassert_equal(read_back, written, "data mismatch: got 0x%08X, want 0x%08X", read_back, written);
}
