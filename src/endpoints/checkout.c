 ...
 * @file checkout.c
 *
 * @brief HTTP endpoint — notifies the server that the user has ended
 *        their workstation session by tapping their RFID/NFC tag.
...
LOG_MODULE_REGISTER(checkout_endpoint, CONFIG_CHECKOUT_ENDPOINT_LOG_LEVEL);
...
int checkout_endpoint_post(int sock, const char *host, const char *port)
{
	...
	ret = HTTP_REQ(HTTP_POST, sock, host, port, "/api/schedule/end", "application/json", body,
		       strlen(body), response_cb, data_response.recv_buf,
		       sizeof(data_response.recv_buf), 5000, NULL);
	if (ret < 0) {
		LOG_ERR("POST /api/schedule/end failed: %d", ret);
		return ret;
	}
	...
	LOG_INF("checkout complete for user %s (HTTP %d)", state.user_id,
